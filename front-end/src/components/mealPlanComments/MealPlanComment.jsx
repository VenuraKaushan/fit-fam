...
const MealPlanComments = ({ postId, commenterName }) => {
    ...
    const handleCommentSubmit = async () => {
        try {
            ...
            const response = await axios.post("http://localhost:8081/mealPlanComments/saveComment", formData, {
                headers: {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get(`http://localhost:8081/mealPlanComments/allComments/${postId}`);
        ...
        const fetchProfilePhoto = async () => {
            try {
                const response = await axios.get(
                    `http://localhost:8081/profile-photo/${commenterName}`
                );
