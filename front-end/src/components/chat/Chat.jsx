...
const Chat = ({ userName, profilePic, sender }) => {
    ...
    const fetchMessages = async () => {
        try {
            const response = await axios.get(`http://localhost:8081/messages/all-messages/${userName}/${sender}`);
    ...
    const handleSendMessage = async () => {
        try {
            ...
            const response = await axios.post('http://localhost:8081/messages/save-message', formData, {
                headers: {
    ...
    const deleteMessage = async (id) => {
        try {
            await axios.delete(`http://localhost:8081/messages/delete-message/${id}`);
            fetchMessages(); // Refresh chat history
        } catch (error) {
