...
const Post = ({ post, userName }) => {
  ...
  useEffect(() => {
    const fetchLikes = async () => {
      try {
        const response = await axios.get(`http://localhost:8081/isLiked/${userName}/${post.id}`);
...
const likePressed = () => {
  const deleteLike = async () => {
    try {
      await axios.delete(`http://localhost:8081/delete/${userName}/${post.id}`);
      setLiked(false);
    } catch (error) {
      console.error('Error fetching data:', error);
    }
  ...
  const setLike = async () => {
    try {
      ...
      const response = await axios.post("http://localhost:8081/setLike", formData, {
        headers: {
...
const updateDescription = async (id, newDescription) => {
    try {
        const response = await axios.patch(
            `http://localhost:8081/descriptionUpdate/${id}/${newDescription}`
        );
