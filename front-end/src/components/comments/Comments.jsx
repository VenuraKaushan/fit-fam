...
const Comments = ({ postId, commenterName, setcomments, numberOfComments }) => {
  ...
  useEffect(() => {
    const fetchComments = async () => {
      try {
        const response = await axios.get(`http://localhost:8081/comments/allComments/${postId}`);
  ...
  const handleCommentSubmit = async () => {
    try {
      ...
      const response = await axios.post("http://localhost:8081/comments/saveComment", formData, {
        headers: {
  ...
  const handleDeleteComment = async (id) => {
    try {
        ...
        await axios.delete(`http://localhost:8081/comments/delete/${id}`);
        setNewComment(""); 
        setcomments(numberOfComments-1);
