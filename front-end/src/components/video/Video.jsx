...
const Video = ({ video, userName }) => {
    ...
    useEffect(() => {
        const fetchLikes = async () => {
          try {
            const response = await axios.get(`http://localhost:8081/api/videos/isLiked/${userName}/${video.id}`);
    ...
    const likePressed = () => {
      const deleteLike = async () => {
        try {
          await axios.delete(`http://localhost:8081/api/videos/delete/${userName}/${video.id}`);
          setLiked(false);
        } catch (error) {
          console.error('Error fetching data:', error);
        }
      ...
      const setLike = async () => {
        try {
          ...
          const response = await axios.post("http://localhost:8081/api/videos/setLike", formData, {
            headers: {
    ...
    const deletePost = async (id) => {
        try {
            const response = await axios.delete(`http://localhost:8081/api/videos/delete/${id}`);
    ...
    const updateDescription = async (id, newDescription) => {
        try {
            const response = await axios.patch(
                `http://localhost:8081/api/videos/descriptionUpdate/${id}/${newDescription}`
            );
