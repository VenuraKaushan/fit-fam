...
const SupplementComment = ({ postId, commenterName }) => {
  ...
  const handleCommentSubmit = async () => {
    try {
      ...
      const response = await axios.post("http://localhost:8081/supplement-comments/saveComment", formData, {
        headers: {
  ...
  const fetchComments = async () => {
    try {
      const response = await axios.get(`http://localhost:8081/supplement-comments/allComments/${postId}`);
