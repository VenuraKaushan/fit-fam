...
const VideoComments = ({ postId, commenterName, setcomments, numberOfComments }) => {
  ...
  useEffect(() => {
    const fetchProfilePhoto = async () => {
      try {
        const response = await axios.get(`http://localhost:8081/profile-photo/${commenterName}`);
  ...
  useEffect(() => {
    const fetchComments = async () => {
      try {
        const response = await axios.get(`http://localhost:8081/VideoComments/allComments/${postId}`);
  ...
  const handleCommentSubmit = async () => {
    try {
      ...
      const response = await axios.post("http://localhost:8081/VideoComments/saveComment", formData, {
        headers: {
  ...
  const handleDeleteComment = async (id) => {
    try {
        ...
        await axios.delete(`http://localhost:8081/VideoComments/delete/${id}`);
        setNewComment(""); 
        setcomments(numberOfComments-1);
